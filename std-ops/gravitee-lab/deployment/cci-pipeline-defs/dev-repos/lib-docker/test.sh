...
docker run -it --name test11ofimage --rm -u root -w /usr/src/giomaven_project "${OCI_REPOSITORY_ORG}/${OCI_REPOSITORY_NAME}:stable-latest" bash -c "pwd && whoami && ls -alh /root && ls -allh /usr/bin/docker && find / -name dockerd && ls -allh /usr/bin/dockerd"
docker run -it --name test11ofimage --rm -u root -w /usr/src/giomaven_project "${OCI_REPOSITORY_ORG}/${OCI_REPOSITORY_NAME}:stable-latest" bash -c "pwd && whoami && /usr/bin/dockerd && docker version"
echo "trying to run docker in docker with circleci user"
docker run -it --name test11ofimage --rm --user ${CCI_USER_UID}:${CCI_USER_GID} -w /usr/src/giomaven_project "${OCI_REPOSITORY_ORG}/${OCI_REPOSITORY_NAME}:stable-latest" bash -c "pwd && whoami && /usr/bin/dockerd && docker version"
