...
export class ProgressMatrix extends rxjs.Subject<ProgressMatrix> { // does not wowrk yet, leave that aside for now
  ...
  public readonly execution_plan: any[][];

  constructor(execution_plan: any[][]){
    super();
...
public next(cciResponse: any) : void {
  ...
  console.log(">>>>>>>>>> Subject NEXT DEBUG [this.progressMatrix.getLength()] =  " + this.matrix.getLength() + "");
  console.log(">>>>>>>>>> Subject NEXT DEBUG [this.progressMatrix.getParallelExecutionSetIndex()] =  " + this.matrix.getParallelExecutionSetIndex() + "");
  console.log(">>>>>>>>>> Subject NEXT DEBUG [this.execution_plan[this.progressMatrix.getParallelExecutionSetIndex()].length] =  " + this.execution_plan[this.matrix.getParallelExecutionSetIndex()].length + "");


  if (this.matrix.getLength() == this.execution_plan[this.matrix.getParallelExecutionSetIndex()].length) {
      console.log(">>>>>>>>>> Subject NEXT >>> <<< ");
      console.log(">>>>>>>>>> Subject NEXT >>> All Pipeline Triggers HTTP Responses have been received from Circle VI API v2 !!! :D  ProgressMatrix is now [" + this.matrix + "] ");
      console.log(">>>>>>>>>> Subject NEXT >>> ProgressMatrix is now : " + this.matrix);
...
error(error: any): void {
   ...
   console.log(error)
   throw new Error(">>>>>>>>>>Subject ERROR for Pipeline Triggers : " + JSON.stringify(error));
}
complete(cciResponse: any) : void {
    ...
    * <ul>
    ...
    start()  : void {
      ...
      for (let parallelExecutionsSetIndex: number = 0; parallelExecutionsSetIndex < arrLength; parallelExecutionsSetIndex++) {
        this.progressMatrix.clear(parallelExecutionsSetIndex);// clearing progress matrix for next {@link ParallelExecutionSet}
    ...
    processExecutionSet(parallelExecutionsSet: string[], parallelExecutionsSetIndex: number) : void {
      ...
      setTimeout((() => { // will interrupt while loop after 10 seconds
        ...
        console.log("[-----------------------------------------------]");
        console.log("[-----------------------------------------------]");
        console.log("[this.stopWatch = true;]");
        console.log("[-----------------------------------------------]");
        console.log("[-----------------------------------------------]");
    ...
    private watchParallelExecutionsSetTriggersProgress(parallelExecutionsSetIndex: number) {
      ...
      console.log('[watchParallelExecutionsSetTriggersProgress] - BEFORE WHILE LOOP => this.progressMatrix.getLength() is ' + this.progressMatrix.getLength());
      ...
      while(this.progressMatrix.getLength() != this.execution_plan[parallelExecutionsSetIndex].length){
        if(this.progressMatrix.getLength() == this.execution_plan[parallelExecutionsSetIndex].length) {
