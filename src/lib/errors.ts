...
class ErrorReporter {
  ...
  constructor(product: string, retries_before_failure: string, ssh_release_git_repo: string, http_release_git_repo: string, release_branches: string) {
    if ((ssh_release_git_repo === undefined || ssh_release_git_repo === "") && (http_release_git_repo === undefined || http_release_git_repo === "")) {
      throw new Error("{[Config]} - Both [HTTP_RELEASE_GIT_REPO] and [SSH_RELEASE_GIT_REPO] are undefined, at least either of them is required. Value should respectively be the SSH URI and the HTTP URI to the git repo versioning the [release.json] file.")
    }

    if (release_branches === undefined || release_branches === "") {
      throw new Error("{[Config]} - [RELEASE_BRANCHES] is undefined, and is required. Its value should be a string, with comma-separated version tags. E.g. : \"master, 3.1.x, 3.0.x, 1.30.x, 1.29.x, 1.25.x, 1.20.x\"");
    }
    ...
    if (product === undefined || product === "") {
      console.warn("{[Config]} - [HTTP_RELEASE_GIT_REPO] is undefined, defaulting value to 'Gravitee APIM'")
      this.product = "Gravitee APIM";
    } else {
      this.product = product;
    }
    if (retries_before_failure === undefined) {
      console.warn("{[Config]} - [RETRY_BEFORE_FAILURE] is undefined, defaulting value to '3'")
      this.retries_before_failure = "3";
    } else {
      if (Number.isNaN(retries_before_failure)) {
        throw new Error("{[Config]} - [RETRY_BEFORE_FAILURE] is defined, but is not a Number. Example value : '7'");
      }
